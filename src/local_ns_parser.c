...
static int local_ns_skiprr(const unsigned char *ptr, const unsigned char *eom,
                ns_sect section, int count);
static int local_ns_dn_skipname(const unsigned char *ptr,
                const unsigned char *eom);
static int local_ns_name_skip(const unsigned char **ptrptr,
                const unsigned char *eom);
...
int local_ns_initparse(const unsigned char *msg, int msglen, ns_msg *handle) {
    ...
    for (i = 0; i < ns_s_max; i++)
        if (handle->_counts[i] == 0)
            ...
            int b = local_ns_skiprr(msg, eom, (ns_sect) i, handle->_counts[i]);
...
int local_ns_parserr(ns_msg *handle, ns_sect section, int rrnum, ns_rr *rr) {
    ...
    /* Do the parse. */
    b = dn_expand(handle->_msg, handle->_eom, handle->LOCAL_NS_MSG_PTR,
                    rr->name, NS_MAXDNAME);
    if (b < 0)
        return (-1);
    handle->LOCAL_NS_MSG_PTR += b;
